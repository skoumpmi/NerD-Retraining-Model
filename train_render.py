...
def inference(args):
    ...
    with train_utils.SetupDirectory(
        ...
        for epoch in range(start_epoch + 1, args.epochs + 1):
            ...
            if epoch % args.validation_epoch == 0 or args.render_only:
            ...
            if epoch % args.testset_epoch == 0 or args.render_only:
                ...
                try:
                    ...
                    print("Directory '%s' created successfully" %testimgdir)
                ...
                for n, t in ret.items():
                    for b in range(t.shape[0]):
                        ...
                        #"""    
                        if "normal" in n or "depth" in n:#el
                            #debugpy.breakpoint()
...
def render_video(
    ...
    kargs = { 'fps': 30, 'quality': 8, 'macro_block_size': None}
    imageio.mimsave(os.path.join(video_dir, "rgb.mp4"), (rgb * 255).astype(np.uint8), 'FFMPEG', **kargs)
    ...
    for n, imgs in parameters.items():
        kargs = { 'fps': 30, 'quality': 8, 'macro_block_size': None}
        imageio.mimsave(os.path.join(video_dir, "{}.mp4".format(n)), (imgs * 255).astype(np.uint8), 'FFMPEG', **kargs)
